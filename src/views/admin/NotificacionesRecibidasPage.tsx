...
import { notificationsService, AdminReceivedNotification } from '@/services/endpoints/notifications'

export function NotificacionesRecibidasPage() {
  const { data: receivedNotifications, isLoading } = useQuery<AdminReceivedNotification[]>({
    queryKey: ['adminReceivedNotifications'],
    queryFn: () => notificationsService.getReceivedNotifications(),
  })

  const [readIds, setReadIds] = useState<string[]>([])

  const markNotificationAsRead = (id_notificacion: string) => {
    setReadIds(prev => [...prev, id_notificacion])
  }
  ...
  if (isLoading) {
    return <div className="text-center py-6 text-surface-muted text-[12px]">Cargando notificaciones...</div>
  }
        ...
        {receivedNotifications?.length === 0 ? (
          ...
          <div className="space-y-3">
            {receivedNotifications?.map((notification) => {
              const isUnread = !notification.leido && !readIds.includes(notification.id_notificacion)
              return (
                <div
                  ...
                  className={cn(
                    ...
                    isUnread ? "border-brand-blue/30" : "opacity-50"
                  )}
                >
                  <div className="flex-1">
                    ...
                    <div className="flex items-center justify-between text-[10px] text-surface-muted mt-1.5">
                      <span className="flex items-center gap-1">
                        <Clock size={10} />
                        {notification.tiempo_transcurrido || notification.created_at}
                      </span>
                    </div>
                  </div>
                  {isUnread && (
